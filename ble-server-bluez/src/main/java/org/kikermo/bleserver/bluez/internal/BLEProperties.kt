...
internal fun BLECharacteristic.toProperties(servicePath: String): Map<String, Map<String, Variant<*>>> {
    println("Characteristic -> getCharacteristicProperties")

    val characteristicProperties =
        buildMap {
...
internal fun BLEService.toProperties(
    isPrimary: Boolean,
    serverName: String,
...
internal fun BLEService.toPath(serverName: String): DBusPath = DBusPath("$PATH_DBUS_DIVIDER$serverName$PATH_DBUS_DIVIDER$name")

internal fun BLECharacteristic.toPath(servicePath: String): String = "$servicePath$PATH_DBUS_DIVIDER$name"
